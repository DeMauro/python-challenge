...
# locate data file with needed information
...
with open(budget_data, 'r', newline='') as csvfile:
    ...
    csv_header = next(csvreader)    #this guy, wha? was with  - print(f"CSV Header: {csv_header}")
    ...
    for row in csvreader:
        ...
        months.append(row[0]) #makes the monts a list so I can use the index of the Min/max above
    i = 0
    for i in range(len(change)): #loop through i to get the list of month or month changes
        if i >=1:
            chchchchange.append(change[i] - change[i -1]) #month over month change I could only figure out as a list
        i += 1
...
little = min(chchchchange) #as above so below
...
print(f'Total: ${profit_net}') #sum of teh profit/loss row
print(f'Average  Change: ${(round(sum(chchchchange)/ncount,2))}') #I looked up sum on the internet. Round too
